...
const Dashboard: React.FC = () => {
  ...
  const { analysisData, logs, years, loading, year, error, fetchData, setYear } = useDashboard();
  ...
  if (!analysisData || loading || isLoading) return <div className="text-center">Loading...</div>;
  ...
  // Pie chart data for budget and spent
  const pieChartData1 = {
          ...
          <ul className="space-y-4">
            {logs.map(log => (
              <li key={log.logId} className="text-gray-700">
                {log.description} - <span className="text-gray-500">{dayjs(log.createdAt).fromNow()}</span>
              </li>
            ))}
          </ul>
        </div>
