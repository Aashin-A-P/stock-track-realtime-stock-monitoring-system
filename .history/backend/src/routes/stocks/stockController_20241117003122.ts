...
export const updateStock = async (req: Request, res: Response) => {
  ...
  export const searchStock = async (req: Request, res: Response) => {
    const { invoiceNo, volNo, pageNo, serialNo, name, stockItemIdentificationNumber, location } = req.query;
    try {
        ...
        if (name) query.where(eq(productsTable.productName, name));
        if (stockItemIdentificationNumber)
            query.where(eq(productsTable.stockItemIdentificationNumber, stockItemIdentificationNumber));
